...
| *KNF02* | R01 | *Response Time* | Sistem harus berhasil mendeteksi dan mengunci koordinat geospasial (latitude, longitude) pengguna dalam waktu maksimal 3 detik setelah formulir pelaporan dibuka|
| *KNF03* | R02 | *Response Time* | Proses pemeriksaan duplikasi berdasarkan radius harus menghasilkan hasil pemeriksaan dalam waktu maksimal 3 detik setelah laporan dikirimkan|
| *KNF04* | R03 | *Reliability* | Setiap laporan yang berhasil diterbitkan memiliki tepat satu ID tiket unik dan tidak menghasilkan tiket ganda akibat pengiriman ulang untuk menjaga konsistensi data laporan|
| *KNF05* | R04 | *Ergonomy* | Dashboard verifikasi harus memungkinkan Tim Administrasi menyortir dan memfilter antrean laporan (berdasarkan kategori, lokasi, status, atau waktu masuk) dalam maksimal 3 klik |
| *KNF06* | R04 | *Security* | Hanya akun dengan peran **Tim Administrasi** yang dapat mengakses antrean verifikasi laporan, sistem harus menolak akses bagi peran lain yang mencoba mengakses endpoint tersebut |
| *KNF07* | R04 | *Response time* | Sistem harus menghitung ulang dan mengurutkan antrean tiket berdasarkan Priority Score dalam waktu maksimal 3 detik setiap kali ada laporan baru yang divalidasi |
| *KNF08* | - | *Reliability* | Proses pembuatan Surat Perintah Kerja (SPK) digital dan pembaruan status tiket menjadi "Dikerjakan" harus dijalankan dalam satu database agar tidak terjadi data SPK fiktif |
| *KNF09* | - | *Ergonomy* | Koordinat lokasi pada aplikasi eksekutor harus terintegrasi langsung dengan Google Maps, sehingga eksekutor tidak perlu mengetik ulang alamat saat mencari jalan menuju lokasi kerusakan.  |
| *KNF10* | - | *Memory* | Dashboard harus menampilkan foto atau video bukti laporan dalam bentuk ukuran kecil (thumbnail) pada tabel , dan baru memuat gambar aslinya saat detail laporan diklik |
| *KNF11* | - | *Availability* | Aplikasi pelaporan warga harus tetap bisa diakses dan menerima laporan 24 jam penuh, meskipun seluruh tim admin sedang dalam status offline (di luar jam kerja). |
| *KNF12* | - | *Security* | Sistem harus dilengkapi dengan fitur verifikasi keamanan (seperti Google reCAPTCHA atau sejenisnya) pada formulir pelaporan warga untuk memastikan laporan dikirim oleh manusia dan mencegah pengiriman otomatis oleh bot. |

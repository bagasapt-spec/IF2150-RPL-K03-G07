...
| :--- | :--- | :--- |
| **KF01** | R02 | Ketika Warga membuka formulir pelaporan, sistem harus mendeteksi dan mengunci koordinat geospasial (*latitude* dan *longitude*) perangkat secara otomatis melalui modul GPS. |
| **KF02** | R02 | Ketika Warga mengakses formulir pelaporan, sistem harus menyediakan menu pilihan kategori kerusakan infrastruktur publik serta kolom uraian deskripsi keluhan. |
| **KF03** | R04 | **Bila** Warga mengunggah berkas foto yang tidak berformat JPG/PNG atau berukuran melebihi 10 MB, **maka sistem harus** menolak berkas tersebut dan menampilkan pesan peringatan validasi. |
| **KF04** | R04 | Ketika formulir pelaporan yang valid dikirimkan, sistem harus menyimpan seluruh data laporan ke basis data dan secara otomatis menetapkan status awal laporan sebagai "Diterima". |
...
| **KF06** | R05 | **Bila** laporan baru berada dalam radius toleransi dari laporan aktif berkategori serupa, **maka sistem harus** menampilkan konfirmasi duplikasi dan mengalihkan aksi pelapor menjadi dukungan (*upvote*). |
...
| **KF08** | R07 | **Bila** pengiriman formulir terindikasi berasal dari *bot* otomatis, **maka sistem harus** memicu verifikasi keamanan (CAPTCHA) dan menahan penyimpanan data hingga verifikasi tuntas. |
...
| **KF20** | R17, R18 | Ketika publik membuka peta sebaran, sistem harus menampilkan peta sebaran (heatmap) seluruh laporan secara real-time dengan menyembunyikan identitas pribadi pelapor (anonymity). |
